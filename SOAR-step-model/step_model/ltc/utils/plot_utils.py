...
def plot_single_video_segmentation(array:np.array, video_info: dict):
    plt.figure()
    ...
    plt.imshow(array)
    plt.axis('off')
    title = f"{video_info['video_name']}"
    if 'len' in video_info:
        title += f", len={video_info['len']}"

    plt.title(title, fontproperties=prop, pad=0.5)
