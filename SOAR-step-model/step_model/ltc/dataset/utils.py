...
def sequence_collate(batch: List[Dict], pad_ignore_idx: int = -100):
    ...
    if 'targets' in batch[0]:
        ...
        if len(batch_target_tensor.shape) == 3:
            batch_size, max_seq_len, _ = batch_feat_tensor.shape
            masks = torch.ones(batch_size, max_seq_len)
        else:
            masks = torch.where(batch_target_tensor == pad_ignore_idx, 0, 1)  # [batch_size, max_seq_len]
        
        masks = masks[:, None, :].bool()
    else:
        masks = torch.ones((features.shape[0], features.shape[-1])).to(features.device)
        masks = masks[:, None, :].bool()
    ...
    if 'targets' in batch[0]:
        batch_dict['targets'] = batch_target_tensor
