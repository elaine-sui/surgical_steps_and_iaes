...
class SingleVideoDataset(Dataset):
    def __init__(self, cfg: CfgNode, video_feats_path: str, active_surgery_pred_path: str):
        self._cfg = cfg
        ...
        if active_surgery_pred_path is not None:
            ...
            end_row_idx = df_active[df_active['label'] == 'activeSurgery'].index.max()
            end_row = df_active.loc[end_row_idx]
            end = int(end_row['duration'].split('-')[1])

            self.start_and_end = {'start': start, 'end': end}
            print(f"Start and end: {self.start_and_end}")

        self._video_meta = {}
        self.video_feats_path = video_feats_path
        self.video_name = os.path.splitext(os.path.split(video_feats_path)[1])[0]
        self._video_sampling_rate = cfg.DATA.FRAME_SAMPLING_RATE
        self._construct_loader()
    ...
    def _construct_loader(self):
        ...
        Construct the list of features and segmentations.
        """
        self._path_to_features = [self.video_feats_path]
        self._video_names = [self.video_name]

    def _load_features(self, feature_path: str):
        features = np.load(feature_path)
        features = features.astype(np.float32).T

        if self.start_and_end is not None: # restrict to detected in-the-body
            features = features[:, self.start_and_end['start']:self.start_and_end['end']]
        features = features[:, ::self._video_sampling_rate]  # [D, T]
        return features
