...
def train_epoch(
    ...
    for cur_iter, (batch_dict) in train_bar:
        ...
        with autocast():
            ...
            if 'uncertainty' in cfg.MODEL.LOSS_FUNC:
                loss_dict = loss_func(logits, batch_dict['targets'], model.uncertainty_log_variance)

                wandb.log({'train/uncertainty_log_variance': model.uncertainty_log_variance.item()}, step=train_meter.global_iter)
            else:
        ...
        if (cur_iter + 1) % cfg.SOLVER.GRAD_ACCUMULATION_STEPS == 0:
            optimizer.step()
            optimizer.zero_grad()
        ...
        if logits.shape[2] == 1: # binary: # [batch_size, seq_len, num_classes]
            prediction = misc.prepare_binary_prediction(logits)
        else:
...
def eval_epoch(
    ...
    # Log epoch stats.
    val_meter.visualize_prediction_result(vis_data=visualization_samples, cur_epoch=cur_epoch, save_visualization=save_visualization, filepath=filepath)
...
def train(cfg: CfgNode):
    ...
    wandb.init(project=cfg.PROJECT_NAME, name=cfg.EXP_PREFIX + "_" + cfg.EXPR_NUM, config=cfg)
    ...
    train_loader, train_dataset = loader.construct_loader(cfg, "train")
    test_loader, test_dataset = loader.construct_loader(cfg, "val")
    ...
    best_metrics_sum = float('-inf') #float('inf')
    eval_metrics = {}
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if misc.is_eval_epoch(cfg, cur_epoch):
            ckpt_dir = cu.get_checkpoint_dir(cfg.OUTPUT_DIR,cfg.EXPR_NUM)
            figures_dir = os.path.join(ckpt_dir, 'validation_figures')
            os.makedirs(figures_dir, exist_ok=True)
            filepath = os.path.join(figures_dir, f'epoch-{cur_epoch:04d}.png')
            eval_metrics = eval_epoch(test_loader, model, val_meter, cur_epoch, device, cfg, save_visualization=True, filepath=filepath)
