...
def extract_intervals(arr):
    ...
    for i, value in enumerate(arr):
        if value > 0.5:
            if start is None:
                start = i  # mark the start of an interval
            
            score += value
        else:
            if start is not None:
                intervals.append((start, i - 1, score / (i - start)))  # add the interval from start to end
                start = None
                score = 0
    
    # handle case where last interval goes to the end of the array
    if start is not None:
        intervals.append((start, len(arr) - 1, score / (len(arr) - start)))
    
    return intervals

def load_pred_and_gt(pred_and_gt_dict):
    # [{'id': id, 'logits': logit.squeeze().to(float), 'pred': prob, 'label': label}]

    pred_vids, pred_starts, pred_stops, pred_labels, pred_scores = [], [], [], [], []
    gt_vids, gt_starts, gt_stops, gt_labels = [], [], [], []
    for item in pred_and_gt_dict:
        # extract intervals
        predicted_intervals = extract_intervals(item['pred'])
        gt_intervals = extract_intervals(item['label'])

        for interval in predicted_intervals:
            pred_vids.append(item['id'])
            pred_starts.append(interval[0])
            pred_stops.append(interval[1])
            pred_labels.append(0)
            pred_scores.append(interval[2])
        
        for interval in gt_intervals:
            gt_vids.append(item['id'])
            gt_starts.append(interval[0])
            gt_stops.append(interval[1])
            gt_labels.append(0)
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.pred_and_gt_dict = pred_and_gt_dict
        self.pred, self.ground_truth = load_pred_and_gt(pred_and_gt_dict)
...
def keyframe_distance(
    ...
    distance_list = list()
    for pred, label, sc_label in zip(
        preds,
        labels,
        sc_labels
    ):
        if sc_label.item() == 1:
            ...
            gt = label
            err_frame = abs(keyframe_loc_pred - gt)
