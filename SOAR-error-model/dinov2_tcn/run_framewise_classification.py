...
def main(args):
    ...
    if args.model in ['DINOv2-base-multiclass', 'DINOv2-base-2head']:
        error_type = args.error_name
    preds = train.predict(model,test_loader,use_bf16=args.use_bf16,error_type=error_type)

    # Reformat predictions
    reformatted_preds = {pred['id']: {'scores': pred['pred']} for pred in preds}
    json_output = {'annotations': reformatted_preds}

    save_path = os.path.join(args.output_dir, args.video_id + ".json")
